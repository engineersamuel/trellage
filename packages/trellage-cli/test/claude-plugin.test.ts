...
describe("readClaudeMarketplace", () => {
  ...
  describe("finalize Claude marketplace seed", () => {
    it("normalizes native plugin state without build paths or transient identity", async () => {
      ...
      await writeFile(path.join(seed, "skills", ".trellage-floating-skills"), "caveman\n")
      await writeFile(path.join(seed, "skills", ".trellage-floating-always-on.md"), "ACTIVE EVERY RESPONSE\n")
      ...
      await expect(readFile(path.join(seed, "skills", ".trellage-floating-skills"), "utf8")).resolves.toBe("caveman\n")
