...
const renderSessionBridgePythonLock = (misePlatform: string): string => {
  const platform =
    misePlatform === "linux-arm64"
      ...
      : {
          checksum: "sha256:6734c3e643c75e860c36ee3a7904e8e6bafbf3232d89b17ffd5fbfa72ab2816c",
...
url = "https://github.com/astral-sh/python-build-standalone/releases/download/20260728/cpython-3.13.14+20260728-${platform.asset}-install_only_stripped.tar.gz"
provenance = "github-attestations"
`
}
...
${renderSessionBridgePythonLock(misePlatform)}
${toolLock}
`
  }
...
const renderHarnessMiseLock = (harness: ReleaseHarnessPackageLock, misePlatform: string): string => {
  ...
  const bridgePythonLock =
    harness.kind === "codex" || harness.kind === "copilot" ? renderSessionBridgePythonLock(misePlatform) : ""
  return `# @generated by Trellage profile compiler
