...
describe("default guide prompts", () => {
  it("offers Headlong and integrates workflow requirements without repetition", async () => {
    ...
    expect(generatePrompt).toContain("For the `customer-engagement-lifecycle` workflow")
    expect(generatePrompt).toContain("DT Coach, Meeting Analyst, BRD Builder, PRD")
    expect(generatePrompt).toContain("Builder, UX UI Designer, ADR Creator, Privacy Planner, RAI Planner, Security")
    expect(generatePrompt).toContain("Planner, SSSC Planner, Functional Planner, and Backlog Manager")
    expect(generatePrompt).toContain("Name only the")
    expect(generatePrompt).toContain("do not force every agent into every candidate")
