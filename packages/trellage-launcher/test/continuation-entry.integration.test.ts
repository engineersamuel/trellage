...
import { beforeAll, expect, test, vi, type TestContext } from "vitest"
import {
  ConversationAgent,
  ConversationRole,
  ConversationSurface,
...
import { runtimeCatalog } from "./helpers/continuation-runtime-fixtures.js"

const repositoryRoot = fileURLToPath(new URL("../../../", import.meta.url))
const bundlePath = path.join(repositoryRoot, "packages/trellage-launcher/dist/launcher.mjs")
...
const snapshot = await captureFocusedConversation(JSON.parse(process.env.TRELLAGE_GUIDE_HERDR_CONTEXT_JSON))
const requestPath = await writeConversationRequest(process.env.HERDR_PLUGIN_STATE_DIR, snapshot)
process.stdout.write(JSON.stringify({ requestPath }))
`

beforeAll(async () => {
  expect(
    (await stat(bundlePath)).isFile(),
    "Build the actual launcher bundle before this integration test.",
  ).toBe(true)
})
  ...
  const server = net.createServer((socket) => {
    ...
    socket.on("data", (chunk: string) => {
      ...
      if (request.method === MetadataMethod.Agent && request.params.target === context.paneId) {
        ...
        request.method === MetadataMethod.Process &&
        request.params.pane_id === context.paneId
      ) {
        result = {
          ...
          process_info: {
            ...
            foreground_processes: [
              { name: "copilot", argv: ["copilot", "--session-id", currentSessionId] },
            ],
          },
        }
      } else {
...
`,
    ...
    await writeFile(
      path.join(transcriptRoot, "workspace.yaml"),
      `id: ${sessionId}\ncwd: ${cwd}\n`,
      { mode: 0o600 },
    )
    await writeFile(
  ...
  return {
    ...
    async capture() {
      const result = await execFileAsync(
        process.execPath,
        ["--input-type=module", "--eval", captureProgram, pluginRoot],
        {
          cwd,
          env,
          timeout: 10_000,
    ...
    start(requestPath: string, model = initialModel, includeHelperRoot = true) {
      const launchEnv = { ...env }
      if (!includeHelperRoot) delete launchEnv.TRELLAGE_GUIDE_CONVERSATION_HELPER_ROOT
...
const createTerminal = (options: {
  ...
  let status: { readonly exitCode: number; readonly signal?: number } | undefined
  let screen = ""
  let output = ""
  ...
  options.onTestFailed(() => {
    console.error(`Bundled launcher screen:\n${screen}\nPTY tail: ${JSON.stringify(output)}`)
  })
  ...
  const waitForText = async (...texts: ReadonlyArray<string>): Promise<void> => {
    await vi.waitFor(() => {
      expect(status, "The bundled launcher exited before review.").toBeUndefined()
...
it("opens installed-style guide assets with the configured helper, saves a real draft, and resumes edits without inference", async ({
  ...
  expect(
    captured.snapshot.messages.map(({ role, text, recordIndex }) => ({ role, text, recordIndex })),
  ).toEqual([
  ...
  expect(
    sourceChecks.filter(({ method }) => method === MetadataMethod.Agent).length,
  ).toBeGreaterThanOrEqual(2)
  expect(
    sourceChecks.filter(({ method }) => method === MetadataMethod.Process).length,
  ).toBeGreaterThanOrEqual(2)
  expect(
    sourceChecks.every(
      ({ id }) => ![String(process.pid), String(ui.pid)].includes(id.split("-")[0] ?? ""),
    ),
  ).toBe(true)
  ...
  expect(await readdir(path.join(fixture.stateRoot, "continuations", "drafts"))).toEqual([
    `${draft.id}.json`,
  ])
  expect(await readdir(path.join(fixture.stateRoot, "continuations", "requests"))).toEqual([])
  expect(
    (await stat(path.join(fixture.stateRoot, "continuations", "drafts", `${draft.id}.json`))).mode &
      0o777,
  ).toBe(0o600)
  expect((await stat(path.join(fixture.stateRoot, "continuations", "drafts"))).mode & 0o777).toBe(
    0o700,
  )
...
it("cannot find the source helper beside installed guide assets without the configured helper root", async ({
  ...
  const ui = fixture.start(captured.requestPath, initialModel, false)
