export var ConversationSurface;
(function (ConversationSurface) {
    ConversationSurface["Host"] = "host";
    ConversationSurface["Native"] = "native";
    ConversationSurface["Sandbox"] = "sandbox";
})(ConversationSurface || (ConversationSurface = {}));
export var ConversationAgent;
(function (ConversationAgent) {
    ConversationAgent["Copilot"] = "copilot";
...
export var ContinuationOutcome;
(function (ContinuationOutcome) {
    ContinuationOutcome["Recommendations"] = "recommendations";
    ContinuationOutcome["NeedsClarification"] = "needs-clarification";
    ContinuationOutcome["NoFurtherAction"] = "no-further-action";
})(ContinuationOutcome || (ContinuationOutcome = {}));
export var ActionImportance;
(function (ActionImportance) {
    ActionImportance["Required"] = "required";
    ActionImportance["Optional"] = "optional";
})(ActionImportance || (ActionImportance = {}));
export var ActionAccess;
(function (ActionAccess) {
    ActionAccess["ReadOnly"] = "read-only";
    ActionAccess["Write"] = "write";
    ActionAccess["Unknown"] = "unknown";
})(ActionAccess || (ActionAccess = {}));
export var ContinuationPlacementKind;
(function (ContinuationPlacementKind) {
    ContinuationPlacementKind["CurrentWorkspacePane"] = "current-workspace-pane";
    ContinuationPlacementKind["NewTab"] = "new-tab";
    ContinuationPlacementKind["NewWorktree"] = "new-worktree";
    ContinuationPlacementKind["ExistingWorktree"] = "existing-worktree";
})(ContinuationPlacementKind || (ContinuationPlacementKind = {}));
export var ContinuationActionStatus;
(function (ContinuationActionStatus) {
    ContinuationActionStatus["Draft"] = "draft";
    ContinuationActionStatus["Prepared"] = "prepared";
    ContinuationActionStatus["Waiting"] = "waiting";
    ContinuationActionStatus["Launching"] = "launching";
    ContinuationActionStatus["Launched"] = "launched";
    ContinuationActionStatus["Failed"] = "failed";
    ContinuationActionStatus["Unknown"] = "unknown";
...
const profilePattern = /^(?:native:[a-z0-9]+(?:-[a-z0-9]+)*\/[a-z0-9]+(?:-[a-z0-9]+)*|sandbox:[a-z0-9]+(?:-[a-z0-9]+)*)$/u;
const object = (value, field, required, optional = []) => {
...
const array = (value, field, maximum, minimum = 0) => {
...
const string = (value, field, maximum, multiline = false) => {
...
const integer = (value, field, maximum = Number.MAX_SAFE_INTEGER) => {
    if (typeof value !== "number" || !Number.isSafeInteger(value) || value < 0 || value > maximum) {
        return invalid(field, "must be a non-negative safe integer within the limit");
    }
    return value;
};
const enumeration = (value, field, members) => {
...
const validateSourceAttachment = (surface, profile, containerId, invocationId) => {
    const field = "source";
...
export const validateConversationSource = (value) => {
    const field = "source";
    ...
    const containerId = fields.containerId === undefined ? undefined : identifier(fields.containerId, `${field}.containerId`);
    const invocationId = fields.invocationId === undefined ? undefined : identifier(fields.invocationId, `${field}.invocationId`);
...
export const conversationSourceKey = (source) => createHash("sha256")
    .update(JSON.stringify(validateConversationSource(source)), "utf8")
    .digest("hex");
export const validateConversationSnapshot = (value) => {
    ...
    unique(messages.map(({ id }) => id), `${field}.messages`);
...
const validateDependencies = (actions) => {
...
const assessment = (value, snapshot, catalogRefs) => {
    ...
    const normalized = (text) => text.normalize("NFKC").trim().replace(/\s+/gu, " ").toLowerCase();
    unique(actions.map(({ id }) => id), `${field}.actions`);
...
export const validateContinuationAssessment = (value, snapshot, catalogRefs) => {
...
const launchReceipt = (value, field) => {
...
const validateActionPreparation = (fields, field, status, launch, prompt) => {
    if (launch !== undefined && launch.status !== status)
        invalid(`${field}.launch`, "status must match the action");
    const inFlight = status === ContinuationActionStatus.Launching ||
        status === ContinuationActionStatus.Launched ||
        status === ContinuationActionStatus.Unknown;
    if (inFlight && launch === undefined)
...
const promptCandidates = (value, field) => {
    ...
    unique(candidates.map(({ id }) => id), `${field}.id`);
    unique(candidates.map(({ prompt }) => prompt.trim()), `${field}.prompt`);
    return candidates;
};
const validateSelectedCandidate = (candidates, selectedCandidateId, status, prompt, field) => {
...
const actionCandidateFields = (fields, field, status, prompt) => {
    const candidates = fields.candidates === undefined ? undefined : promptCandidates(fields.candidates, `${field}.candidates`);
    const selectedCandidateId = fields.selectedCandidateId === undefined
        ? undefined
...
const actionConfirmations = (fields, field) => ({
...
export const validateContinuationDraft = (value) => {
    const field = "draft";
    ...
    unique(actions.flatMap(({ launch }) => (launch === undefined ? [] : [launch.attemptId])), `${field}.actions.launch.attemptId`);
