...
import { ConversationRole, type ConversationMessage } from "@trellage/guide-core/conversation"
import { sourceWorkspaceRoot } from "@trellage/runtime"

import { hasErrorCode, isRecord, type JsonRecord } from "./records.ts"

const execFileAsync = promisify(execFile)
export const sandboxBridgeMaximumOutputBytes = 512 * 1024

interface SandboxFinalMessageRunnerInput {
  readonly identity: SandboxConversationIdentity
  readonly cwd?: string | undefined
  readonly env: NodeJS.ProcessEnv
}

const parseBridgeResult = (source: string, identity: SandboxConversationIdentity) => {
  let value: unknown
  try {
  ...
  return { answer: value.answer, session_id: value.session_id }
}

const defaultBridgeRunner = async ({ identity, env, cwd }: SandboxFinalMessageRunnerInput) => {
  const repositoryRoot = env.TRELLAGE_ROOT ?? sourceWorkspaceRoot()
...
export const captureSandboxFinalMessage = async ({
  ...
  readonly bridgeRunner?: (input: SandboxFinalMessageRunnerInput) => Promise<string>
}) => {
...
export interface SandboxConversationIdentity {
...
export interface SandboxConversationRunnerInput {
  readonly identity: SandboxConversationIdentity
  readonly cwd?: string | undefined
  readonly env: NodeJS.ProcessEnv
...
export interface SandboxConversationOptions {
  readonly identity: SandboxConversationIdentity
  readonly cwd?: string | undefined
  readonly env?: NodeJS.ProcessEnv | undefined
  readonly signal?: AbortSignal | undefined
...
export interface SandboxBridgeProcessOptions {
...
interface ConversationBridgeResponse extends JsonRecord {
...
interface ConversationBridgeMetadata {
  ...
  readonly coverage: { readonly complete: boolean; readonly notices: string[] }
}

interface SandboxConversationPage extends ConversationBridgeResponse, ConversationBridgeMetadata {
  readonly snapshotId: string
  readonly capturedAt: string
  readonly messages: unknown[]
  readonly page: { readonly index: number; readonly total: number; readonly nextCursor: string | null }
}
...
function requireObjectKeys(value: unknown, keys: ReadonlyArray<string>): asserts value is JsonRecord {
  if (
    !isRecord(value) ||
...
const parseConversationResponse = (source: unknown, identity: SandboxConversationIdentity): ConversationBridgeResponse => {
  ...
  const identityKeys = ["agent", "profile", "containerId", "invocationId"] as const
  if (
...
function validateConversationMetadata(value: JsonRecord): asserts value is JsonRecord & ConversationBridgeMetadata {
  ...
  if (
    ...
    typeof value.cutoff.recordIndex !== "number" || !Number.isSafeInteger(value.cutoff.recordIndex) ||
...
function validateConversationPage(value: ConversationBridgeResponse): asserts value is SandboxConversationPage {
  ...
  validateConversationPagePosition(value.snapshotId, value.page)
}

const validateConversationPagePosition = (snapshotId: string, page: JsonRecord) => {
  const { index, total, nextCursor } = page
  if (
  ...
  if (index === total - 1) {
    ...
    !nextCursor.startsWith(snapshotId)
  ) {
    throw new Error("The Sandbox conversation page is missing its next cursor.")
  }
}

const normalizedConversationMessage = (value: unknown): ConversationMessage => {
  requireObjectKeys(value, ["id", "role", "text", "recordIndex"])
  if (
    typeof value.id !== "string" || !messageIdPattern.test(value.id) ||
    (value.role !== ConversationRole.User && value.role !== ConversationRole.Assistant) ||
    typeof value.text !== "string" || value.text.trim().length === 0 ||
    typeof value.recordIndex !== "number" || !Number.isSafeInteger(value.recordIndex) || value.recordIndex < 0
  ) {
...
const pageMetadata = (page: SandboxConversationPage) => JSON.stringify({
...
class SandboxConversationPages {
  first: SandboxConversationPage | undefined
  ...
  accept(page: ConversationBridgeResponse) {
  ...
  result() {
    ...
    if (
      ...
      last?.role !== ConversationRole.Assistant || last.id !== first.cutoff.messageId ||
