import fs, { constants, type NoParamCallback, type PathLike, type Stats } from "node:fs"
...
import { hasErrorCode, isRecord } from "./records.ts"
...
const owned = (status: Stats, mode: number) =>
...
const privateDirectory = async (directory: string, create: boolean): Promise<void> => {
  try {
    ...
    if (!create || !hasErrorCode(error, "ENOENT")) throw error
    await assertNoConversationSymlinks(path.dirname(directory))
    try {
      await mkdir(directory, { mode: 0o700 })
    } catch (creationError) {
      if (!hasErrorCode(creationError, "EEXIST")) throw creationError
    }
...
const checkedFile = (status: Stats) => {
...
const writeLockedJson = async (directory: string, value: unknown) => {
  ...
  const release = await properLockfile.lock(directory, {
    ...
    fs: {
      ...fs,
      mkdir: (target: PathLike, callback: NoParamCallback) => fs.mkdir(target, { mode: 0o700 }, callback),
    },
