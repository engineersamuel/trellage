...
PRE_PUSH_TEST_TARGETS := publication-contract publication-contract-self-test agent-profile-hup-contract caveman-profile-contract profile-compiler trellage-identity trellage-orphan-cleanup agent-harness claude-entry native-claude-profile manifest contract adapter awesome-adapter copilot-image runner session workspace-checks playwright-matrix evidence
