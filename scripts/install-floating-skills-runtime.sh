...
[[ "$#" == 0 ]] || {
  printf 'Usage: install-floating-skills-runtime.sh [--stage ABSOLUTE_PATH]\n' >&2
  exit 2
}
common_root="$("${trellage_bun[@]}" "$repo_root/packages/trellage-runtime/src/workspace-cli.ts" \
  ensure-common "$canonical_home")"
exec "$repo_root/scripts/install-source-runtime.sh" --install-floating \
  "$common_root/floating-skills-runtime"
