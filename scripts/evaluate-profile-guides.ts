...
interface CatalogProfile {
  ...
  readonly workflows: ReadonlyMap<string, CompactWorkflow>
  readonly prerequisites: readonly unknown[]
  readonly headless: unknown
  readonly herdrCompatibility: unknown
...
const rank = (value: unknown, name: string) => {
...
const expectedWorkflowMap = (value: unknown, name: string, expectedProfiles: readonly string[]) => {
...
export const parseProfileGuideScenarios = (value: unknown): Scenario[] => {
...
const loadScenarios = async () =>
  parseProfileGuideScenarios(
    JSON.parse(await readFile(path.join(root, "tests", "fixtures", "profile-guide-scenarios.json"), "utf8")),
  )
...
const runCommand = (
  ...
  new Promise<string>((resolve, reject) => {
    ...
    const finishAfterProcessTreeExit = (error: Error): void => {
      if (!leaderClosed || processTreeIsRunning()) {
        setTimeout(() => finishAfterProcessTreeExit(error), 25)
        return
      }
      finish(error)
    }
    const terminate = (error: Error) => {
      ...
      forceKill = setTimeout(() => {
        ...
        finishAfterProcessTreeExit(error)
      }, 2_000)
    }
...
const runMiseTask = (task: string, taskArgs: readonly string[], env?: NodeJS.ProcessEnv, input?: string) =>
...
const compactWorkflow = (value: unknown, name: string): CompactWorkflow => {
...
const catalogProfile = (value: unknown, surface: string): [string, CatalogProfile] => {
  ...
  const workflows = new Map(
    guide.workflows.map((workflow, index): [string, CompactWorkflow] => {
...
const profileCatalogIndex = (nativeList: ProfileList, sandboxList: ProfileList) => {
  const entries = [
...
const validateNativeCatalogSync = (nativeList: ProfileList, sourceDescriptions: ReadonlyMap<string, string>) => {
  const installedProfiles = new Set<string>()
...
const validateExpectedWorkflowProfiles = (
...
const validateRecommendationConfidence = (
...
const validateRecommendationEnrichment = (
  recommendation: Record<string, unknown>,
...
const evaluate = async () => {
  ...
  try {
    ...
    const nativeValue = record(JSON.parse(nativeSource), "Native profile list")
    const sandboxValue = record(JSON.parse(sandboxSource), "Sandbox profile list")
    if (!Array.isArray(nativeValue.profiles) || !Array.isArray(sandboxValue.profiles)) {
      throw new Error("worktree profile lists must contain profiles")
    }
    const nativeList: ProfileList = { profiles: nativeValue.profiles }
    const sandboxList: ProfileList = { profiles: sandboxValue.profiles }
...
if (import.meta.main) {
  ...
  if (args.length !== 1 || args[0] !== "--live") {
    process.stderr.write("Usage: bun scripts/evaluate-profile-guides.ts --live\n")
