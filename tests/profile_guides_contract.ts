...
for (const family of nativeFamilies) {
  ...
  if (launchers.length !== 1 || launcher === undefined)
    throw new Error(`${family}/bin must contain exactly one launcher`)
...
const isOperationalExample = (example: string) => operationalExamplePatterns.some((pattern) => pattern.test(example))
...
const socialExamples = new Set(socialGuide.guide.workflows.flatMap(({ examples }) => examples))
for (const example of [
  "Build my voice",
...
const persistentInvestigation = headlongGuide.guide.workflows.find(({ id }) => id === "persistent-investigation")
...
const expectedEccWorkflows = new Map<string, readonly [string, string]>([
...
const normalizeSemanticText = (value: string) =>
  value
    ...
    .replace(/\s+/gu, " ")
    .trim()
const workflowSemanticText = ({ description, examples }: ProfileGuideWorkflow) =>
  normalizeSemanticText([description, ...examples].join(" "))
type SemanticGroups = ReadonlyArray<ReadonlyArray<RegExp>>
const hasSemanticGroupsNear = (value: string, anchor: string, groups: SemanticGroups, radius = 260) => {
...
requireWorkflowSemanticGroups("lfg", [
  [
    ...
    [[/\brequirements ready\b/u, /\bsettled requirements?\b/u, /\bapproved plans?\b/u, /\bexisting plans?\b/u]],
  ],
  ["hands-off execution", [[/\bhands off\b/u, /\bautonomous\b/u, /\bwithout check ins?\b/u, /\bno check ins?\b/u]]],
  [
  ...
  [
    ...
    [[/\bwithout a remote\b/u, /\bno remote\b/u], [/\blocal\b/u], [/\bcommits?\b/u]],
  ],
])
requireWorkflowSemanticGroups("ce-compound", [
  ["one learning per run", [[/\bone\b/u], [/\bper run\b/u]]],
  [
...
const hasCompoundBodyTermsNear = (anchor: string, terms: ReadonlyArray<RegExp>, radius = 220) => {
...
const compoundBodyFacts = [
  ...
  [
    ...
    hasCompoundBodyTermsNear(
      "/ce-setup",
      [/\b(?:first|once)\b/u, /\brepositor(?:y|ies)\b/u, /\bmanual(?:ly)?\b/u],
      320,
    ),
  ],
  ...
  [
    ...
    hasCompoundBodyTermsNear("same-session", [/\bhandoff\b/u, /\bce-plan\b/u, /\blfg\b/u]),
  ],
  [
  ...
  [
    ...
    hasCompoundBodyTermsNear(
      "/ce-plan",
      [/\binteractive\b/u, /\boutside\b/u, /\bheadless\b/u, /\bright-size\b/u, /\bdirect\b/u, /\bchat brief\b/u],
      320,
    ),
  ],
  [
  ...
  [
    ...
    hasCompoundBodyTermsNear(
      "completed",
      [/\beach\b/u, /\bunit\b/u, /\b(?:help|helps|make|makes|improve|improves)\b/u, /\bnext\b/u],
      180,
    ),
  ],
  [
    "repository-local solution knowledge",
    hasCompoundBodyTermsNear("repository-local", [/\bsolutions?\b/u, /\bknowledge\b/u], 180),
  ],
  [
  ...
  [
    ...
    hasCompoundBodyTermsNear(
      "stack-land",
      [/\bexplicit\b/u, /\bauthoriz(?:e|es|ing)\b/u, /\b(?:land|lands|landing|merge|merges|merging)\b/u],
      320,
    ),
  ],
  [
    ...
    hasCompoundBodyTermsNear(
      "/lfg",
      [/\bce-brainstorm\b/u, /\brequirements?\b/u, /\b(?:ready|settled|approved)\b/u],
      520,
    ),
  ],
  [
    ...
    hasCompoundBodyTermsNear(
      "numbered choices",
      [/\bchat\b/u, /\bblocking question tool\b/u, /\bnext\b/u, /\bturn\b/u],
      360,
    ),
  ],
  [
  ...
  [
    ...
    hasCompoundBodyTermsNear(
      "implementation-only",
      [/\bstop\b/u, /\bcommit\b/u, /\bpush\b/u, /\bmode:return-to-caller\b/u, /\bce-work\b/u],
      420,
    ),
  ],
  [
...
const implementationWorkflow = graphGuide.guide.workflows.find(({ id }) => id === "implement-complex-change")
if (
...
const validationWorkflow = graphGuide.guide.workflows.find(({ id }) => id === "validate-existing-implementation")
if (
