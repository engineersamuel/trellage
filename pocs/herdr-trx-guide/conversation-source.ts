...
import { captureFocusedConversation, sameConversationSource } from "./lib/conversation-capture.ts"
...
export const main = async (
  ...
  signal?.throwIfAborted()
  if (args.length !== 2 || !["--check", "--refresh"].includes(args[0]) || !env.HERDR_PLUGIN_STATE_DIR) {
    throw new Error("Use conversation-source.ts --check SNAPSHOT_PATH or --refresh SNAPSHOT_PATH with plugin state.")
  }
  const previous = await readRequest(env.HERDR_PLUGIN_STATE_DIR, args[1])
  signal?.throwIfAborted()
  if (args[0] === "--check") return checkConversationSource(previous, { env, capture, signal })
  const current = await capture({ ...previous.source, expectedSource: previous.source }, { env, signal })
  signal?.throwIfAborted()
  ...
  const requestPath = await stageRequest(env.HERDR_PLUGIN_STATE_DIR, current)
  signal?.throwIfAborted()
  return { requestPath }
}
