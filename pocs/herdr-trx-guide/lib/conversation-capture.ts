...
const supportedAgent = (value: unknown): ConversationSource["agent"] => {
  if (!Object.values(ConversationAgent).includes(value as ConversationSource["agent"])) {
    throw new ConversationSourceError("The focused pane is not a supported Copilot, Codex, or Claude conversation.")
  }
  return value as ConversationSource["agent"]
}
...
export const conversationServerId = async (env: NodeJS.ProcessEnv): Promise<string> => {
  ...
  return `herdr-${createHash("sha256").update(JSON.stringify([
    await realpath(socket), status.dev, status.ino, status.birthtimeMs,
  ])).digest("hex")}`
}
...
const checkedAgent = (agentInfo, processInfo, context: FocusedConversationContext) => {
...
const exactSandboxSessionId = (agent, agentInfo, processInfo) => {
  const reference = agentInfo.agent_session
  if (reference !== undefined &&
    (reference?.agent !== agent || reference?.kind !== "id")) {
...
const makeSandboxBinding = (base, identity, agentInfo, processInfo): FocusedConversationBinding => {
...
const makeLocalBinding = (base, identity, transcript): FocusedConversationBinding => ({
  ...base,
...
const bindFocusedSource = async (
  ...
  assertBinding(context.binding, binding)
  assertBinding(context.expectedSource, binding)
  dependencies.signal?.throwIfAborted()
  return { binding, transcript, identity }
}
...
const captureBoundSandbox = async (bound, dependencies: FocusedCaptureDependencies, env) => {
...
const captureBoundLocal = async (bound, dependencies: FocusedCaptureDependencies) => {
...
export const captureFocusedConversation = async (
  ...
  const { source, normalized } = bound.binding.surface === ConversationSurface.Sandbox
