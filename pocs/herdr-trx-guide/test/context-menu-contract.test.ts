...
import {
  ...
  VisibleMessageSelectionError,
} from "../lib/visible-message.ts";
...
const contextJson = (agent = "copilot", extra: Record<string, unknown> = {}) =>
  JSON.stringify({
...
const runAction = async (
  ...
  options: {
    ...
    readonly capture?: (
      context: ReturnType<typeof parseInvocationContext>,
  ...
  await writeFile(
    ...
    [
      JSON.stringify({
        ...
        data: { summary: "# Long exact answer\n\nThe task completion document." },
      }),
...
for (const scenarioName of [
  "invalid session with no root",
  "conflicting process sessions with no root",
  "unsafe exact transcript",
]) {
  test(`does not fall back after ${scenarioName}`, async (t) => {
    ...
    const scenario = [
      {
        name: "invalid session with no root",
        home: path.join(root, "absent"),
        id: "invalid/id",
        processes: [],
      },
      {
        name: "conflicting process sessions with no root",
        home: path.join(root, "absent"),
        id: "session-1",
        processes: [
    ...
    ].find((candidate) => candidate.name === scenarioName);
    assert.ok(scenario);
    let visibleReads = 0;
    const captured = await captureContextMenuRequest({
...
test("captures the latest answer for direct actions with ordinary terminal text selected", async (t) => {
  ...
  const { result, launches, requests } = await runAction(stateDir, {
    context: contextJson("copilot", {
      invocation_source: "pane",
      selected_text: "ordinary terminal text",
    }),
    capture: async () => captured,
    choiceConsumer: async () => {
      throw new Error("Terminal selection is not a picker token");
    },
  });
...
test("offers Rewrite output first and captures its source once when the picker opens", () => {
  ...
  const unavailable = contextMenuSourceChoice({
    ...request(),
    kind: "context-menu-error",
    message: undefined,
    error: { code: "missing", message: "No visible message" },
  });
...
test("routes a frozen picker choice through an opaque one-use token and private popup request", async (t) => {
  ...
  const picked = request({
    message: { ...request().message!, text: "Captured before pane changed" },
  });
  ...
  const { result, launches } = await runAction(stateDir, {
...
test("rejects a staged choice when the active pane identity changed", async (t) => {
  ...
  t.after(() => rm(stateDir, { recursive: true, force: true }));
  const token = await writeChoice(stateDir, {
    schemaVersion: 1,
    kind: "context-menu",
    request: request(),
  });
...
test("surfaces missing pane, missing message, truncation, and ambiguous viewport failures", async () => {
  const context = parseInvocationContext(contextJson());
  const capture = (read: Record<string, unknown>) =>
    captureContextMenuRequest({
      context,
...
test("validates the request and binds its message to the exact pane", () => {
  ...
  assert.equal(
    contextMenuSourceMatches(
      request(),
      parseInvocationContext(contextJson("copilot", { focused_pane_cwd: "/other" })),
    ),
    false,
  );
  assert.deepEqual(
    contextMenuRequestFromChoice({ schemaVersion: 1, kind: "context-menu", request: request() }),
    request(),
  );
  assert.throws(
    () =>
      parseContextMenuRequest({
        ...request(),
        message: { ...request().message!, paneId: "other" },
      }),
    /different pane/u,
  );
});
    ...
    socketPath: await (async () => {
      ...
      const server = net.createServer((socket) => {
        ...
        socket.on("data", (chunk) => {
          ...
          socket.end(
            `${JSON.stringify({ id: request.id, result: { type: "pane_read", read: { pane_id: "pane-1", workspace_id: "workspace-1", tab_id: "tab-1", source: "visible", format: "text", text: "● Visible answer", captured_at: "2026-09-10T12:00:00.000Z" } } })}\n`,
          );
        });
      });
  ...
  assert.throws(
    () =>
      selectLatestVisibleMessage({
        paneId: "pane-1",
        agent: "copilot",
        text: "A message without a turn marker",
      }),
