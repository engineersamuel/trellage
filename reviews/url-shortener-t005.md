...
with line numbers instead. Every finding it returned was then reproduced by hand
against a live server before being accepted or rejected — two reproduced, one did
not hold.
...
**Fix:** percent-encode the URL on its way into `Location`
(`quote(url, safe=_LOCATION_SAFE)`). CR and LF become `%0D%0A`; already-safe
URLs, including their reserved characters (`?`, `&`, `#`, `/`, `:`), are
unchanged. This deliberately does *not* validate what the URL means — the spec
lists URL validation as a non-goal — it only makes the value safe to emit.
...
**Fix:** the same percent-encoding as finding 1 (UTF-8 percent-encoding is the
correct wire form for a non-ASCII URI in `Location`).
...
instances. The spec already narrows the guarantee to *within one `Store` instance*
(`docs/url-shortener-spec.md`, "returns the same code, within one `Store`
instance"), and persistence is an explicit non-goal. The code matches the spec as
written. No change.

Worth knowing for later: codes are the first 6 characters of a SHA-256-derived
alphabet mapping, so the birthday bound puts the first likely collision somewhere
around 10^5 stored URLs. Below that, collision probing effectively never fires.
...
- `git stash push shortener/handlers.py` → `tests.test_review_findings` fails 3 of 4
  (header injection, non-Latin-1 crash, oversized body timeout). The fourth test
  guards that ordinary URLs pass through `Location` untouched, and passes either way.
