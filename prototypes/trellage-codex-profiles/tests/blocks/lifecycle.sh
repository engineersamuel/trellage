...
# Block B: profile lifecycle. Depends on durable `pstack` and `superpowers`
...
mv "$fixture_root/snapshot-session.saved" "$pstack_home/sessions/keep.jsonl"

# Repeated setup must read both inventories and never mutate native plugin state.
: >"$fixture_root/fake-codex.log"
write_isolation_snapshot setup-pstack-ordinary
HOME="$fixture_root/home" fake_env "$fixture_launcher" setup pstack \
  >"$fixture_root/setup-pstack-idempotent.out" || fail 'idempotent setup pstack failed'
assert_isolation_snapshot_unchanged setup-pstack-ordinary
jq -se '
  length == 4
  and map(.args) == [
...
jq -se --arg pstack "$pstack_home" --arg superpowers "$superpowers_home" '
  length == 8
  and all(.[0:4][]; .codexHome == $pstack)
...
assert_lifecycle_waits_for_lifecycle() {
  ...
  HOME="$fixture_root/home" PATH="$fake_bin:$PATH" \
    ...
    "$fixture_launcher" setup pstack >"$hold_dir/holder.out" 2>&1 &
  holder_pid=$!
...
assert_lifecycle_reports_lock_holder() {
  ...
  holder_group="$(cat "$hold_dir/first-child.pid")"
  track_async_group "$holder_group"
  lock_owner="$(awk '{print $1}' "$pstack_home/.launch.lock")"

  HOME="$fixture_root/home" PATH="$fake_bin:$PATH" \
    FAKE_CODEX_LOG="$fixture_root/fake-codex.log" \
    "$fixture_launcher" doctor pstack >"$hold_dir/contender.out" 2>&1 &
  contender_pid=$!
  track_async_pid "$contender_pid"
  wait_count=0
  ...
  grep -q "waiting for profile lock: pstack is held by pid $lock_owner" \
...
assert_lifecycle_waits_for_lifecycle doctor doctor pstack
assert_lifecycle_waits_for_lifecycle update update pstack
assert_lifecycle_waits_for_lifecycle setup setup pstack
assert_lifecycle_waits_for_lifecycle repair repair pstack
assert_lifecycle_waits_for_lifecycle update-check update --check pstack
...
HOME="$fixture_root/home" PATH="$fake_bin:$PATH" \
  ...
  "$fixture_launcher" pstack --version >"$concurrent_launch_dir/first.out" 2>&1 &
...
HOME="$fixture_root/home" PATH="$fake_bin:$PATH" \
  FAKE_CODEX_LOG="$fixture_root/fake-codex.log" \
  FAKE_CODEX_OVERLAP_DIR="$concurrent_launch_dir" \
  "$fixture_launcher" pstack --version >"$concurrent_launch_dir/second.out" 2>&1 &
...
cp "$fixture_root/config-before-temp-die.toml" "$pstack_home/config.toml"
...
valid_pstack_plugin="$fixture_root/valid-pstack-plugin.json"
jq -cn --arg path "$pstack_home/.tmp/marketplaces/pstack-for-codex-local" \
  '{installed:[{pluginId:"pstack-for-codex@pstack-for-codex-local",name:"pstack-for-codex",marketplaceName:"pstack-for-codex-local",version:"0.1.0",installed:true,enabled:true,source:{source:"local",path:$path},marketplaceSource:{sourceType:"git",source:"https://github.com/Aqua-123/pstack-for-codex.git"},installPolicy:"AVAILABLE",authPolicy:"ON_INSTALL"}],available:[]}' \
  >"$valid_pstack_plugin"

assert_selected_plugin_rejected() {
  plugin_label="$1"
  plugin_filter="$2"
  jq "$plugin_filter" "$valid_pstack_plugin" >"$inventory_bad" \
...
assert_selected_plugin_rejected wrong-version '.installed[0].version = "9.9.9"'
assert_selected_plugin_rejected wrong-version-type '.installed[0].version = 1'
assert_selected_plugin_rejected wrong-plugin-id '.installed[0].pluginId = "other@pstack-for-codex-local"'
...
assert_selected_plugin_path_rejected() {
  ...
  jq --arg installedPath "$installed_path" '.installed[0].installedPath = $installedPath' \
    "$valid_pstack_plugin" >"$inventory_bad" \
...
mkdir -p "$pstack_home/../outside-plugin" "$fixture_root/external-plugin"
ln -s "$fixture_root/external-plugin" "$pstack_home/plugin-link"
assert_selected_plugin_path_rejected traversal-installed-path "$pstack_home/../outside-plugin"
assert_selected_plugin_path_rejected symlink-installed-path "$pstack_home/plugin-link"
assert_selected_plugin_path_rejected missing-installed-path "$pstack_home/missing-plugin"
rm "$pstack_home/plugin-link"

printf '%s\n' '{"marketplaces":[{"name":"pstack-for-codex-local","root":"/wrong","marketplaceSource":{"sourceType":"git","source":"/wrong"}}]}' >"$inventory_bad"
: >"$fixture_root/fake-codex.log"
FAKE_CODEX_MARKETPLACE_OVERRIDE="$inventory_bad" \
...
rm "$fake_state/pstack/forbidden-superpowers-direct"
...
jq '.installed[0].version = "wrong" | .installed += [{pluginId:"superpowers",name:"superpowers",marketplaceName:null,version:"6.2.0",installed:true,enabled:false,source:{source:"git",url:"obra/superpowers"},marketplaceSource:null,installPolicy:"AVAILABLE",authPolicy:"ON_INSTALL"}]' \
  "$valid_pstack_plugin" >"$atomic_bad_plugin"
: >"$fake_state/pstack/forbidden-superpowers-direct"
: >"$fixture_root/fake-codex.log"
...
for marketplace_case in missing duplicate wrong-name wrong-kind; do
  case "$marketplace_case" in
    missing) printf '%s\n' '{"marketplaces":[]}' >"$inventory_bad" ;;
    duplicate) jq -cn --arg root "$pstack_home/.tmp/marketplaces/pstack-for-codex-local" '{marketplaces:[{name:"pstack-for-codex-local",root:$root,marketplaceSource:{sourceType:"git",source:"https://github.com/Aqua-123/pstack-for-codex.git"}},{name:"pstack-for-codex-local",root:$root,marketplaceSource:{sourceType:"git",source:"https://github.com/Aqua-123/pstack-for-codex.git"}}]}' >"$inventory_bad" ;;
    wrong-name) jq -cn --arg root "$pstack_home/.tmp/marketplaces/pstack-for-codex-local" '{marketplaces:[{name:"other",root:$root,marketplaceSource:{sourceType:"git",source:"https://github.com/Aqua-123/pstack-for-codex.git"}}]}' >"$inventory_bad" ;;
    wrong-kind) jq -cn --arg root "$pstack_home/.tmp/marketplaces/pstack-for-codex-local" '{marketplaces:[{name:"pstack-for-codex-local",root:$root,marketplaceSource:{sourceType:"local",source:$root}}]}' >"$inventory_bad" ;;
  esac
  : >"$fixture_root/fake-codex.log"
  FAKE_CODEX_MARKETPLACE_OVERRIDE="$inventory_bad" \
...
jq '.installed[0].marketplaceSource.source = "/wrong"' "$valid_pstack_plugin" \
  >"$inventory_bad"
: >"$fixture_root/fake-codex.log"
FAKE_CODEX_PLUGIN_OVERRIDE="$inventory_bad" \
...
pstack_marketplace_saved="$fixture_root/pstack-marketplace-saved"
mv "$pstack_home/.tmp/marketplaces/pstack-for-codex-local" "$pstack_marketplace_saved"
printf '%s\n' '{"installed":{}}' >"$inventory_bad"
: >"$fixture_root/fake-codex.log"
FAKE_CODEX_PLUGIN_OVERRIDE="$inventory_bad" \
  assert_command_fails preflight-before-mutation env HOME="$fixture_root/home" \
    PATH="$fake_bin:$PATH" FAKE_CODEX_LOG="$fixture_root/fake-codex.log" \
    "$fixture_launcher" setup pstack
[ ! -e "$pstack_home/.tmp/marketplaces/pstack-for-codex-local" ] \
  || fail 'malformed later inventory added marketplace'
jq -se 'all(.[]; (.args | join(" ") | test(" add | remove | upgrade ") | not))' \
  "$fixture_root/fake-codex.log" >/dev/null || fail 'preflight failure mutated lifecycle state'
mv "$pstack_marketplace_saved" "$pstack_home/.tmp/marketplaces/pstack-for-codex-local"

# Repair may replace one selected invalid plugin, but preserves unrelated profile state.
mkdir -p "$pstack_home/sessions" "$pstack_home/plugins/unrelated"
printf '%s\n' 'session bytes' >"$pstack_home/sessions/keep.jsonl"
printf '%s\n' 'unrelated plugin bytes' >"$pstack_home/plugins/unrelated/state"
cp "$pstack_home/sessions/keep.jsonl" "$fixture_root/session-before-repair"
cp "$pstack_home/plugins/unrelated/state" "$fixture_root/plugin-before-repair"
jq '.installed[0].version = "9.9.9" | .installed[0].enabled = false' \
  "$valid_pstack_plugin" >"$inventory_bad"
: >"$fixture_root/fake-codex.log"
env HOME="$fixture_root/home" PATH="$fake_bin:$PATH" \
  FAKE_CODEX_LOG="$fixture_root/fake-codex.log" \
  FAKE_CODEX_PLUGIN_OVERRIDE="$inventory_bad" FAKE_CODEX_PLUGIN_OVERRIDE_ONCE=1 \
  "$fixture_launcher" repair pstack >"$fixture_root/repair-wrong-plugin.out" \
...
cmp -s "$fixture_root/session-before-repair" "$pstack_home/sessions/keep.jsonl" \
  || fail 'repair changed session bytes'
cmp -s "$fixture_root/plugin-before-repair" "$pstack_home/plugins/unrelated/state" \
...
mv "$pstack_home/config.toml" "$fixture_root/pstack-config-saved.toml"
assert_status 2 update-check-missing-config env HOME="$fixture_root/home" PATH="$fake_bin:$PATH" \
  FAKE_CODEX_LOG="$fixture_root/fake-codex.log" "$fixture_launcher" update --check pstack
mv "$fixture_root/pstack-config-saved.toml" "$pstack_home/config.toml"

printf '%s\n' 'external config' >"$fixture_root/external-config.toml"
mv "$pstack_home/config.toml" "$fixture_root/pstack-config-saved.toml"
ln -s "$fixture_root/external-config.toml" "$pstack_home/config.toml"
assert_status 2 update-check-symlink-config env HOME="$fixture_root/home" PATH="$fake_bin:$PATH" \
  FAKE_CODEX_LOG="$fixture_root/fake-codex.log" "$fixture_launcher" update --check pstack
rm "$pstack_home/config.toml"
mv "$fixture_root/pstack-config-saved.toml" "$pstack_home/config.toml"

mv "$pstack_home" "$fixture_root/pstack-home-saved"
assert_status 2 update-check-missing-home env HOME="$fixture_root/home" PATH="$fake_bin:$PATH" \
  FAKE_CODEX_LOG="$fixture_root/fake-codex.log" "$fixture_launcher" update --check pstack
mv "$fixture_root/pstack-home-saved" "$pstack_home"
...
assert_status 0 update-check-current env HOME="$fixture_root/home" PATH="$fake_bin:$PATH" \
  ...
  "$fixture_launcher" update --check superpowers
...
HOME="$fixture_root/home" fake_env "$fixture_launcher" update pstack \
  >"$fixture_root/update-pstack.out" || fail 'update pstack failed'
write_isolation_snapshot update-pstack-ordinary-after
grep -Fv $'pstack.config\t' "$fixture_root/update-pstack-ordinary.state-before" \
  >"$fixture_root/update-pstack-ordinary.filtered-before"
grep -Fv $'pstack.config\t' "$fixture_root/update-pstack-ordinary-after.state-before" \
  >"$fixture_root/update-pstack-ordinary.filtered-after"
cmp -s "$fixture_root/update-pstack-ordinary.filtered-before" \
  "$fixture_root/update-pstack-ordinary.filtered-after" \
  || fail 'pstack update changed session, MCP, plugin, or other-profile state'
[ "$(grep -Fxc '[plugins."pstack-for-codex@pstack-for-codex-local"]' \
  "$pstack_home/config.toml")" -eq 1 ] \
  || fail 'pstack update did not re-add exactly one selected native plugin table'
grep -F -A1 -- '[plugins."unrelated@user-owned"]' "$pstack_home/config.toml" \
  | grep -F -- 'enabled = false' >/dev/null \
  || fail 'pstack update changed unrelated native plugin state'
jq -se '
  [ .[] | select(
      .args[1] == "remove" or .args[1] == "add" or .args[2] == "upgrade"
    ) | .args ] == [
...
' "$fixture_root/fake-codex.log" >/dev/null || fail 'pstack update mutation order differs'
auth_is_absent "$fixture_root/home/.codex/auth.json" \
  || fail 'update created host authentication'
auth_is_absent "$pstack_home/auth.json" || fail 'update created profile authentication'
...
# Failed pstack reinstall leaves a diagnosable missing plugin; repair restores only it.
FAKE_CODEX_FAIL_MUTATION=plugin-add \
  assert_status 2 update-pstack-failed env HOME="$fixture_root/home" PATH="$fake_bin:$PATH" \
    FAKE_CODEX_LOG="$fixture_root/fake-codex.log" "$fixture_launcher" update pstack
grep -F -- 'native plugin add failed: simulated Codex diagnostic' \
  "$fixture_root/update-pstack-failed.out" >/dev/null \
  || fail 'failed pstack reinstall hid native Codex stderr'
[ ! -e "$fake_state/pstack/plugin" ] || fail 'failed pstack reinstall did not leave plugin missing'
assert_status 2 update-check-missing-plugin env HOME="$fixture_root/home" PATH="$fake_bin:$PATH" \
  FAKE_CODEX_LOG="$fixture_root/fake-codex.log" "$fixture_launcher" update --check pstack
...
[ -e "$fake_state/pstack/plugin" ] || fail 'repair did not restore missing pstack plugin'
auth_is_absent "$fixture_root/home/.codex/auth.json" \
  || fail 'repair created host authentication'
auth_is_absent "$pstack_home/auth.json" || fail 'repair created profile authentication'
...
rm -f "$fixture_root/home/.codex/auth.json" "$pstack_home/auth.json"
native_config_hash="$(state_file_hash "$pstack_home/config.toml")"
: >"$fixture_root/fake-codex.log"
...
assert_command_fails native-login-unavailable env HOME="$fixture_root/home" \
  PATH="$fake_bin:$PATH" FAKE_CODEX_LOG="$fixture_root/fake-codex.log" \
  FAKE_CODEX_LOGIN_STATUS=1 "$fixture_launcher" --native-auth pstack --version
...
[ "$(state_file_hash "$pstack_home/config.toml")" = "$native_config_hash" ] \
...
assert_command_fails native-source-missing env HOME="$fixture_root/home" \
  PATH="$fake_bin:$PATH" FAKE_CODEX_LOG="$fixture_root/fake-codex.log" \
  FAKE_CODEX_LOGIN_STATUS=0 "$fixture_launcher" --native-auth pstack --version
assert_isolation_snapshot_unchanged native-source-missing
grep -F -- 'pstack' "$fixture_root/native-source-missing.out" >/dev/null \
...
chmod 0640 "$pstack_home/auth.json"
malformed_target_hash="$(state_file_hash "$pstack_home/auth.json")"
malformed_target_inode="$(file_inode "$pstack_home/auth.json")"
malformed_target_mode="$(file_mode "$pstack_home/auth.json")"
malformed_config_hash="$(state_file_hash "$pstack_home/config.toml")"
: >"$fixture_root/fake-codex.log"
assert_command_fails native-malformed-host-auth env HOME="$fixture_root/home" \
  PATH="$fake_bin:$PATH" FAKE_CODEX_LOG="$fixture_root/fake-codex.log" \
  FAKE_CODEX_LOGIN_STATUS=1 "$fixture_launcher" --native-auth pstack --version
grep -F -- 'pstack' "$fixture_root/native-malformed-host-auth.out" >/dev/null \
  || fail 'malformed host auth diagnostic omitted selected profile'
[ "$(state_file_hash "$pstack_home/auth.json")" = "$malformed_target_hash" ] \
  || fail 'malformed host auth changed profile authentication bytes'
[ "$(file_inode "$pstack_home/auth.json")" = "$malformed_target_inode" ] \
  || fail 'malformed host auth replaced profile authentication'
[ "$(file_mode "$pstack_home/auth.json")" = "$malformed_target_mode" ] \
  || fail 'malformed host auth changed profile authentication mode'
[ "$(state_file_hash "$pstack_home/config.toml")" = "$malformed_config_hash" ] \
...
cmp -s "$fixture_root/home/.codex/auth.json" "$pstack_home/auth.json" \
  || fail 'native-auth launch did not copy exact host authentication'
[ "$(file_mode "$pstack_home/auth.json")" = '600' ] \
...
auth_inode="$(file_inode "$pstack_home/auth.json")"
chmod 0400 "$pstack_home/auth.json"
: >"$fixture_root/fake-codex.log"
HOME="$fixture_root/home" FAKE_CODEX_LOGIN_STATUS=0 \
  fake_env "$fixture_launcher" --native-auth pstack --version \
  || fail 'identical native-auth launch failed'
[ "$(file_inode "$pstack_home/auth.json")" = "$auth_inode" ] \
...
assert_native_refresh_failure() {
  ...
  shift
  prior_hash="$(state_file_hash "$pstack_home/auth.json")"
  prior_inode="$(file_inode "$pstack_home/auth.json")"
  prior_mode="$(file_mode "$pstack_home/auth.json")"
  : >"$fixture_root/fake-codex.log"
  ...
  [ "$(state_file_hash "$pstack_home/auth.json")" = "$prior_hash" ] \
    || fail "$label changed destination authentication bytes"
  [ "$(file_inode "$pstack_home/auth.json")" = "$prior_inode" ] \
    || fail "$label changed destination authentication inode"
  [ "$(file_mode "$pstack_home/auth.json")" = "$prior_mode" ] \
...
mv "$pstack_home/auth.json" "$fixture_root/native-auth-target.saved"
ln -s "$fixture_root/native-auth-target.saved" "$pstack_home/auth.json"
assert_native_refresh_failure native-target-symlink env HOME="$fixture_root/home" \
  PATH="$fake_bin:$PATH" FAKE_CODEX_LOG="$fixture_root/fake-codex.log" \
  FAKE_CODEX_LOGIN_STATUS=0 "$fixture_launcher" --native-auth pstack --version
rm "$pstack_home/auth.json"
mv "$fixture_root/native-auth-target.saved" "$pstack_home/auth.json"

mv "$pstack_home/auth.json" "$fixture_root/native-auth-target.saved"
mkdir "$pstack_home/auth.json"
...
assert_command_fails native-target-nonregular env HOME="$fixture_root/home" \
  PATH="$fake_bin:$PATH" FAKE_CODEX_LOG="$fixture_root/fake-codex.log" \
  FAKE_CODEX_LOGIN_STATUS=0 "$fixture_launcher" --native-auth pstack --version
grep -F -- 'pstack' "$fixture_root/native-target-nonregular.out" >/dev/null \
...
: >"$fixture_root/fake-codex.log"
cleanup_prior_hash="$(state_file_hash "$pstack_home/auth.json")"
cleanup_prior_inode="$(file_inode "$pstack_home/auth.json")"
...
[ "$(state_file_hash "$pstack_home/auth.json")" = "$cleanup_prior_hash" ] \
  || fail 'cleanup failure changed destination authentication bytes'
[ "$(file_inode "$pstack_home/auth.json")" = "$cleanup_prior_inode" ] \
  || fail 'cleanup failure changed destination authentication inode'
[ "$(file_mode "$pstack_home/auth.json")" = "$cleanup_prior_mode" ] \
  || fail 'cleanup failure changed destination authentication mode'
stale_auth_stage="$(find "$pstack_home" -maxdepth 1 -name '.auth.*' -print -quit)"
...
for native_signal in HUP INT TERM; do
  ...
  grep -F -- "native authentication refresh interrupted for profile pstack: $native_signal" \
...
EOF
chmod +x "$fake_bin/cp"
native_target_hash="$(state_file_hash "$pstack_home/auth.json")"
native_target_inode="$(file_inode "$pstack_home/auth.json")"
: >"$fixture_root/fake-codex.log"
...
mv "$fixture_root/native-auth-post-stage.saved" "$pstack_home/auth.json"
...
profile_auth_hash="$(shasum -a 256 "$pstack_home/auth.json" | awk '{print $1}')"
profile_auth_inode="$(file_inode "$pstack_home/auth.json")"
: >"$fixture_root/fake-codex.log"
...
[ "$(shasum -a 256 "$pstack_home/auth.json" | awk '{print $1}')" = "$profile_auth_hash" ] \
  || fail 'default launch changed profile authentication bytes'
[ "$(file_inode "$pstack_home/auth.json")" = "$profile_auth_inode" ] \
