...
enum StagePhase {
  ...
  PublishedHashes = "published-hashes",
  PublishedMarker = "published-marker",
...
interface JsonObjectFrame {
  readonly kind: JsonFrameKind.Object
  readonly keys: Set<string>
  state: JsonFrameState
}

interface JsonArrayFrame {
  readonly kind: JsonFrameKind.Array
  state: JsonFrameState
}
...
interface SeedIdentity extends FileIdentity {
...
interface LockOwner extends Owner {
  readonly schema: 1
...
interface LockRecord extends LockOwner {
  readonly seed: SeedIdentity
  readonly private: FileIdentity & { readonly name: string }
}
...
interface StageOwner extends Owner {
  readonly schema: 1
  readonly seed: unknown
...
interface StageRecord extends StageOwner {
  readonly seed: SeedIdentity
...
interface ManagedDirectory {
  readonly path: string
  readonly kind: ManagedKind.Directory
  readonly mode: string
}

interface ManagedFile {
  readonly path: string
  readonly kind: ManagedKind.File
  readonly mode: string
  readonly sha256: string
}

type ManagedEntry = ManagedDirectory | ManagedFile

interface ManagedInput {
  readonly absolute: string
  readonly relative: string
  readonly installedRelative: string
  readonly status: Stats
}
...
interface InventoryContext {
  ...
  readonly forbiddenBuildPaths: readonly string[]
  readonly plugin: string
  readonly expectedVersion: string
  readonly seen: Set<string>
  readonly managedEntries: ManagedEntry[]
  foundManifest: string | undefined
}

interface GenericInventoryContext {
  readonly root: string
  readonly forbiddenBuildPaths: readonly string[]
  readonly entries: ManagedFile[]
}
...
interface ManagedSettings {
  readonly extraKnownMarketplaces: Record<string, { source: { source: "github"; repo: string } }>
...
interface PrepareOptions {
  ...
  readonly plugin: string
  readonly forbiddenBuildPaths: readonly string[]
  readonly expectedSourceArgument: string
  readonly expectedSource: string
  readonly expectedVersion: string
  readonly managed: ManagedSettings
}

const outputNames = ["managed-settings.json", "managed-files.txt", "managed.sha256", "managed-lock.json"] as const
type OutputName = (typeof outputNames)[number]
const outputNameSet = new Set<string>(outputNames)
...
const stagePhases = new Set<string>(Object.values(StagePhase))
const jsonWhitespace = new Set([" ", "\t", "\n", "\r"])
const maxJsonCharacters = 1_000_000
const maxJsonDepth = 128
const authoritativeManifests = new Set([path.join(".github", "plugin", "plugin.json"), "plugin.json"])
const unsupportedAlternateManifests = [path.join(".copilot", "plugin.json"), path.join(".claude-plugin", "plugin.json")]
...
const pathIsMissing = async (file: string): Promise<boolean> => {
  try {
    await lstat(file)
    return false
  } catch (error) {
...
const optionalStatus = async (file: string): Promise<Stats | undefined> => {
  try {
...
const optionalRegularFile = async (
  file: string,
  label: string,
): Promise<{ file: string; mode: number } | undefined> => {
  try {
...
const assertExactJson = (actual: unknown, expected: unknown, label: string): void => {
...
const sameKeys = (value: unknown, expected: readonly string[]): value is Record<string, unknown> =>
  isRecord(value) && JSON.stringify(Object.keys(value).sort(lexical)) === JSON.stringify([...expected].sort(lexical))

const sameIdentity = (left: unknown, right: SeedIdentity): boolean =>
  isRecord(left) && left.path === right.path && left.dev === right.dev && left.ino === right.ino
...
const readStrictJson = async (handle: FileHandle, message: string): Promise<unknown> => {
  try {
...
const skipJsonWhitespace = (parser: JsonParser): void => {
  while (parser.index < parser.source.length && jsonWhitespace.has(parser.source.charAt(parser.index))) {
    parser.index += 1
  }
}

const jsonEscapeEnd = (source: string, cursor: number, message: string): number => {
...
const readJsonStringToken = (parser: JsonParser, message: string): { value: string; end: number } => {
  if (parser.source[parser.index] !== '"') fail(message)
  let cursor = parser.index + 1
  while (cursor < parser.source.length) {
    const character = parser.source.charAt(cursor)
    if (character === '"') {
      const end = cursor + 1
      const value: unknown = JSON.parse(parser.source.slice(parser.index, end))
      if (typeof value !== "string") fail(message)
      return { value, end }
    }
    if (character.charCodeAt(0) < 0x20) fail(message)
    cursor = character === "\\" ? jsonEscapeEnd(parser.source, cursor + 1, message) : cursor + 1
  }
  fail(message)
}

const pushJsonFrame = (parser: JsonParser, kind: JsonFrameKind, state: JsonFrameState, message: string): void => {
  if (parser.frames.length >= maxJsonDepth) fail(message)
  parser.frames.push(kind === JsonFrameKind.Object ? { kind, keys: new Set<string>(), state } : { kind, state })
  parser.index += 1
}

const readJsonValue = (parser: JsonParser, message: string): void => {
  const character = parser.source[parser.index]
  if (character === "{") return pushJsonFrame(parser, JsonFrameKind.Object, JsonFrameState.KeyOrEnd, message)
  if (character === "[") return pushJsonFrame(parser, JsonFrameKind.Array, JsonFrameState.ValueOrEnd, message)
  if (character === '"') {
...
const readJsonObjectKey = (parser: JsonParser, frame: JsonObjectFrame, allowEnd: boolean, message: string): void => {
  ...
  frame.state = JsonFrameState.Colon
  parser.index = key.end
}

const readJsonObjectEnd = (parser: JsonParser, frame: JsonObjectFrame, message: string): void => {
  if (parser.source[parser.index] === ",") {
    parser.index += 1
    frame.state = JsonFrameState.Key
    return
  }
  if (parser.source[parser.index] !== "}") fail(message)
  closeJsonFrame(parser)
...
const advanceJsonObject = (parser: JsonParser, frame: JsonObjectFrame, message: string): void => {
  switch (frame.state) {
    case JsonFrameState.KeyOrEnd:
      return readJsonObjectKey(parser, frame, true, message)
    case JsonFrameState.Key:
      return readJsonObjectKey(parser, frame, false, message)
    case JsonFrameState.Colon:
      if (parser.source[parser.index] !== ":") fail(message)
      parser.index += 1
      frame.state = JsonFrameState.Value
      return
    case JsonFrameState.Value:
      frame.state = JsonFrameState.CommaOrEnd
...
const readJsonArrayValue = (parser: JsonParser, frame: JsonArrayFrame, allowEnd: boolean, message: string): void => {
  if (parser.source[parser.index] === "]") {
    if (!allowEnd) fail(message)
    closeJsonFrame(parser)
    return
...
const readJsonArrayEnd = (parser: JsonParser, frame: JsonArrayFrame, message: string): void => {
  if (parser.source[parser.index] === ",") {
    parser.index += 1
    frame.state = JsonFrameState.Value
    return
...
const advanceJsonArray = (parser: JsonParser, frame: JsonArrayFrame, message: string): void => {
  switch (frame.state) {
    case JsonFrameState.ValueOrEnd:
...
const assertNoDuplicateJsonKeys = (source: string, message: string): void => {
  if (source.length > maxJsonCharacters) fail(message)
  const parser: JsonParser = { source, index: 0, frames: [], rootState: JsonRootState.Value }
  while (true) {
    skipJsonWhitespace(parser)
    const frame = parser.frames.at(-1)
    if (frame !== undefined) {
      if (frame.kind === JsonFrameKind.Object) advanceJsonObject(parser, frame, message)
      else advanceJsonArray(parser, frame, message)
      continue
    }
    if (parser.rootState === JsonRootState.End) {
      if (parser.index !== source.length) fail(message)
      return
    }
    parser.rootState = JsonRootState.End
    readJsonValue(parser, message)
  }
}

const readOptionalJson = async (handle: FileHandle): Promise<unknown> => {
  try {
    return JSON.parse(await handle.readFile("utf8"))
  } catch (error) {
...
const stableFileStatus = (opened: Stats, expected: Stats): boolean =>
  opened.isFile() && opened.dev === expected.dev && opened.ino === expected.ino

const lockPrefix = (kind: LockKind): string => (kind === LockKind.Main ? privateLockPrefix : privateRecoveryPrefix)

const validLockOwner = (record: unknown, kind: LockKind): record is LockOwner =>
...
const validLockSeed = (record: LockOwner, seedIdentity: SeedIdentity): boolean =>
  sameKeys(record.seed, ["path", "dev", "ino"]) && sameIdentity(record.seed, seedIdentity)

const validLockPrivateReference = (record: LockOwner, prefix: string, name: unknown, status: Stats): boolean =>
...
const validOwnedLockRecord = (
  record: unknown,
  kind: LockKind,
  seedIdentity: SeedIdentity,
  status: Stats,
): record is LockRecord =>
  validLockOwner(record, kind) &&
  validLockSeed(record, seedIdentity) &&
  validLockPrivateReference(
    record,
    lockPrefix(kind),
    isRecord(record.private) ? record.private.name : undefined,
    status,
  )

const lockWasReleased = async (error: unknown, publicPath: string, label: string): Promise<boolean> => {
  if (!isMissing(error)) throw error
  if (await pathIsMissing(publicPath)) return true
  fail(`invalid ${label}`)
}

const openOwnedPublicLock = async (publicPath: string, label: string): Promise<OpenedFile | undefined> => {
...
const validateOpenedOwnedLock = async (
  opened: Stats,
  expected: Stats,
  publicPath: string,
  label: string,
...
const validateOwnedPrivateLock = async (
  privatePath: string,
  opened: Stats,
  publicPath: string,
  label: string,
...
const createOwnedLock = async (
  ...
  const prefix = kind === LockKind.Main ? privateLockPrefix : privateRecoveryPrefix
...
const privateLockDescriptor = (name: string): LockDescriptor | undefined => {
  if (name.startsWith(privateLockPrefix)) return { kind: LockKind.Main, prefix: privateLockPrefix }
  if (name.startsWith(privateRecoveryPrefix)) {
    return { kind: LockKind.Recovery, prefix: privateRecoveryPrefix }
  }
  return undefined
}

const openOrphanPrivateLock = async (privatePath: string): Promise<OpenedFile | undefined> => {
...
const readOrphanPrivateLock = async (privatePath: string): Promise<{ opened: Stats; record: unknown } | undefined> => {
...
const removeConfirmedOrphanPrivateLock = async (privatePath: string, opened: Stats): Promise<void> => {
  const confirmed = await lstat(privatePath)
  if (stableFileStatus(confirmed, opened) && confirmed.nlink === 1) await rm(privatePath)
}

const cleanupOrphanPrivateLock = async (buildRoot: string, seedIdentity: SeedIdentity, name: string): Promise<void> => {
...
const cleanupOrphanPrivateLocks = async (buildRoot: string, seedIdentity: SeedIdentity): Promise<void> => {
...
const validStageOwner = (record: unknown): record is StageOwner =>
...
const validStageSeed = (record: StageOwner, seedIdentity: SeedIdentity): boolean =>
  sameKeys(record.seed, ["path", "dev", "ino"]) && sameIdentity(record.seed, seedIdentity)

const validStageLocation = (record: StageOwner, name: string, prefix: string, status: Stats): boolean =>
  name === `${prefix}${record.nonce}` &&
...
const openOptionalStageState = async (statePath: string, stateStatus: Stats): Promise<unknown> => {
  let handle: FileHandle
  try {
...
const privateStageEntriesAreValid = async (stage: string): Promise<boolean> => {
...
const readPrivateStageState = async (
  buildRoot: string,
  seedIdentity: SeedIdentity,
  name: string,
): Promise<{ stage: string; status: Stats; record: StageRecord } | undefined> => {
...
const cleanupOrphanPrivateStages = async (buildRoot: string, seedIdentity: SeedIdentity): Promise<void> => {
  const names = (await readdir(buildRoot)).filter((name) => name.startsWith(privateStagePrefix)).sort(lexical)
  for (const name of names) {
    ...
    if (
      ...
      confirmed.dev === owned.status.dev &&
      confirmed.ino === owned.status.ino
    ) {
...
const clearStaleRecoveryLock = async (
  seed: string,
  seedIdentity: SeedIdentity,
  buildRoot: string,
): Promise<boolean> => {
  const recovery = await readOwnedLock(path.join(seed, recoveryName), LockKind.Recovery, seedIdentity, buildRoot)
  if (recovery === undefined) return true
  if (processIsAlive(recovery.record.pid)) return false
  await removeOwnedLock(recovery, LockKind.Recovery, seedIdentity, buildRoot)
  return true
}
...
const recoverExistingLock = async (
  seed: string,
  seedIdentity: SeedIdentity,
  buildRoot: string,
  deadline: number,
  ...
  try {
    const currentRecovery = await readOwnedLock(recovery.publicPath, LockKind.Recovery, seedIdentity, buildRoot)
    if (currentRecovery === undefined || currentRecovery.record.nonce !== recovery.record.nonce) {
      fail("lost recovery lock ownership")
    }
    const lock = await readOwnedLock(path.join(seed, lockName), LockKind.Main, seedIdentity, buildRoot)
    if (lock === undefined) return true
    if (processIsAlive(lock.record.pid)) return false
    const confirmedRecovery = await readOwnedLock(recovery.publicPath, LockKind.Recovery, seedIdentity, buildRoot)
    if (confirmedRecovery === undefined || confirmedRecovery.record.nonce !== recovery.record.nonce) {
      fail("lost recovery lock ownership")
    }
    await removeOwnedLock(lock, LockKind.Main, seedIdentity, buildRoot)
    return true
  } finally {
    await releaseOwnedLock(recovery, LockKind.Recovery, seedIdentity, buildRoot)
  }
}

const acquireLock = async (seed: string, seedIdentity: SeedIdentity, buildRoot: string): Promise<OwnedLock> => {
...
const validatePluginManifest = (content: Buffer, plugin: string, expectedVersion: string): void => {
  ...
  if (!isRecord(parsed)) fail("installed plugin manifest is invalid")
...
const recordManagedDirectory = async (
...
const managedFileUnchanged = (after: Stats, opened: Stats): boolean =>
  stableFileStatus(after, opened) && after.nlink === 1 && after.size === opened.size && after.mtimeMs === opened.mtimeMs

const readManagedFile = async (
  absolute: string,
  relative: string,
  status: Stats,
): Promise<{ content: Buffer; opened: Stats }> => {
  const handle = await open(absolute, constants.O_RDONLY | constants.O_NOFOLLOW)
  try {
    const opened = await handle.stat()
...
const recordManagedFile = async (context: InventoryContext, entry: ManagedInput): Promise<void> => {
...
const registerManagedPath = (context: InventoryContext, relative: string, installedRelative: string): void => {
  if (hasControlCharacter(relative)) {
...
const readManagedEntry = async (context: InventoryContext, directory: string, entry: Dirent): Promise<ManagedInput> => {
...
const inventoryManagedEntry = async (context: InventoryContext, directory: string, child: Dirent): Promise<void> => {
...
const inventoryManagedDirectory = async (context: InventoryContext, directory: string): Promise<void> => {
...
const inventoryPlugin = async (
  seed: string,
  installed: string,
  forbiddenBuildPaths: readonly string[],
  plugin: string,
  ...
  if (!installedStatus.isDirectory() || installedStatus.isSymbolicLink())
...
const validateGenericSkillPath = (relative: string): void => {
  if (hasControlCharacter(relative)) fail(`unsafe generic skill path: ${relative}`)
...
const recordGenericSkillFile = async (
  context: GenericInventoryContext,
  absolute: string,
  relative: string,
  status: Stats,
): Promise<void> => {
...
const inventoryGenericSkillEntry = async (
  context: GenericInventoryContext,
  directory: string,
  ...
  const bakedEvidence = [".trellage-floating-skills", ".trellage-floating-always-on.md"].includes(relative)
  if (bakedEvidence && !status.isFile()) fail(`invalid baked skill evidence: ${relative}`)
  if (!status.isDirectory() && !status.isFile()) fail(`unsupported generic skill entry: ${relative}`)
  if (!inside(context.root, await realpath(absolute))) {
    fail(`generic skill path escapes seed: ${relative}`)
  }
  if (bakedEvidence) return
...
const inventoryGenericSkillDirectory = async (
  context: GenericInventoryContext,
  directory: string,
  relativeDirectory: string,
): Promise<void> => {
...
const inventoryGenericSkills = async (seed: string, forbiddenBuildPaths: readonly string[]): Promise<ManagedFile[]> => {
  ...
  const context: GenericInventoryContext = { root, forbiddenBuildPaths, entries: [] }
  for (const name of await readdir(skills)) {
    if (![".trellage-floating-skills", ".trellage-floating-always-on.md"].includes(name))
      safeIdentifier(name, "generic skill")
  }
...
const inventoryInstructionFiles = async (
  seed: string,
  forbiddenBuildPaths: readonly string[],
): Promise<ManagedFile[]> => {
...
const inventoryGenericInstructions = async (
  seed: string,
  forbiddenBuildPaths: readonly string[],
): Promise<ManagedFile[]> => {
...
function invalidStage(name: string): never {
  return fail(`invalid finalization stage: ${name}`)
}

const readRequiredStageRecord = async (stage: string, name: string): Promise<unknown> => {
  const statePath = path.join(stage, "state.json")
  try {
...
const validatePublicStageEntries = async (stage: string, name: string): Promise<void> => {
  const allowed = new Set(["state.json", "state.next.json", ...outputNames])
  for (const entry of await readdir(stage)) {
...
const readStageState = async (
  seed: string,
  seedIdentity: SeedIdentity,
  name: string,
): Promise<{ stage: string; record: StageRecord }> => {
...
const recoverStages = async (seed: string, seedIdentity: SeedIdentity): Promise<void> => {
  const names = await readdir(seed)
  for (const name of names.filter((candidate) => candidate.startsWith(stagePrefix)).sort(lexical)) {
    if (hasControlCharacter(name)) fail(`control character in finalization stage path: ${JSON.stringify(name)}`)
...
const removeEmptyCopilotPluginLock = async (seed: string): Promise<void> => {
...
const nativeMarketplacePath = (settings: unknown, marketplace: string): string => {
  const marketplaces =
    isRecord(settings) && isRecord(settings.extraKnownMarketplaces) ? settings.extraKnownMarketplaces : undefined
  const entry = marketplaces?.[marketplace]
  const localSource = isRecord(entry) ? entry.source : undefined
  if (!isRecord(localSource) || localSource.source !== "directory") fail("native marketplace source is not local")
  if (
...
const validateNativeMarketplacePath = async (
  localPath: string,
  expectedSourceArgument: string,
...
const readMarketplaceManifest = async (expectedSource: string): Promise<Record<string, unknown>> => {
...
const marketplacePluginEntry = (
  ...
  const plugins: unknown[] = manifest.plugins
  const matches = plugins.filter(isRecord).filter((entry) => entry.name === plugin)
  const match = matches[0]
  if (matches.length !== 1 || match === undefined) fail("native marketplace plugin source is missing or ambiguous")
  if (match.version !== expectedVersion) fail("native marketplace plugin version does not match the lock")
  return match
}

const marketplacePluginRootSegments = (metadata: unknown): string[] => {
  ...
  if (
    ...
    pluginRoot.length === 0 ||
    hasControlCharacter(pluginRoot) ||
    pluginRoot.includes("\\") ||
...
const marketplacePluginSourceSegments = (source: unknown): string[] => {
  if (source === ".") return []
  if (
...
const resolveMarketplacePluginSource = async (
  expectedSource: string,
  marketplace: string,
  plugin: string,
  expectedVersion: string,
...
const ensureDirectory = async (directory: string, label: string): Promise<void> => {
  try {
...
const beginDereferencedDirectory = async (
  directory: string,
  allowedRoot: string,
...
const validateDereferencedSymlink = async (absolute: string, context: DereferenceContext): Promise<void> => {
  const followed = await stat(absolute)
  if (!followed.isFile() && !followed.isDirectory()) {
    fail("live plugin symlink target must be a regular file or directory")
  }
  ...
  if (status.isDirectory()) {
    await validateDereferencedDirectory(target, context.allowedRoot, context.ancestors, context.visited)
    return
  }
  if (!status.isFile()) fail("live plugin symlink target must be a regular file or directory")
}

const validateDereferencedEntry = async (
  absolute: string,
  status: Stats,
  ...
  if (status.isDirectory()) {
    await validateDereferencedDirectory(absolute, context.allowedRoot, context.ancestors, context.visited)
    return
  }
...
const validateDereferencedDirectory = async (
  ...
  const nextAncestors = await beginDereferencedDirectory(directory, allowedRoot, ancestors, visited)
...
const validateDereferencedPlugin = async (sourcePlugin: string, allowedRoot: string): Promise<void> => {
  await validateDereferencedDirectory(sourcePlugin, allowedRoot, new Set<string>(), new Set<string>())
}

const normalizeCopiedPluginDirectory = async (directory: string): Promise<void> => {
...
const materializeLivePlugin = async (
  seed: string,
  expectedSource: string,
  marketplace: string,
  plugin: string,
  ...
  const sourcePluginReal = await resolveMarketplacePluginSource(expectedSource, marketplace, plugin, expectedVersion)
  try {
...
const requireExactDirectory = async (directory: string, expected: readonly string[], label: string): Promise<void> => {
  const status = await lstat(directory)
  if (!status.isDirectory() || status.isSymbolicLink()) fail(`${label} must be a directory`)
...
const validateSeedEntryName = (name: string): void => {
...
const readSeedEntryStatus = async (seed: string, name: string): Promise<Stats | undefined> => {
  try {
...
const validateSeedDirectory = (name: string, status: Stats): void => {
  if (!status.isDirectory() || status.isSymbolicLink()) fail(`${name} must be a directory`)
}

const expectedSeedLinks = (name: string): number => (name === lockName || name === recoveryName ? 2 : 1)

const validateRetainedSeedFile = async (
  absolute: string,
  name: string,
  forbiddenBuildPaths: readonly string[],
): Promise<void> => {
...
const validateSeedEntry = async (seed: string, name: string, forbiddenBuildPaths: readonly string[]): Promise<void> => {
...
const validateInstalledPluginTree = async (seed: string, marketplace: string, plugin: string): Promise<string> => {
  const installedRoot = path.join(seed, "installed-plugins")
  const marketplaceRoot = path.join(installedRoot, marketplace)
  const installed = path.join(marketplaceRoot, plugin)
...
const prepareInstalledPlugin = async ({
  ...
  if (settingsFile === undefined) {
    ...
    assertExactJson(
      JSON.parse(await readFile(path.join(seed, "managed-settings.json"), "utf8")),
      managed,
...
const publishFinalizedSeed = async (
  seed: string,
  buildRoot: string,
  seedIdentity: SeedIdentity,
  owner: Owner,
  buffers: Record<OutputName, Buffer>,
): Promise<void> => {
  ...
  let published = false
  try {
    await writeStageState(privateStage, seedIdentity, stageIdentity, owner, StagePhase.Staged)
    ...
    const phases: Record<OutputName, StagePhase> = {
      "managed-settings.json": StagePhase.PublishedSettings,
      "managed-files.txt": StagePhase.PublishedFiles,
      "managed.sha256": StagePhase.PublishedHashes,
      "managed-lock.json": StagePhase.PublishedMarker,
    }
    for (const name of outputNames) {
      await rename(path.join(stage, name), path.join(seed, name))
      await writeStageState(stage, seedIdentity, stageIdentity, owner, phases[name])
    }
    await rm(path.join(seed, "settings.json"), { force: true })
    await writeStageState(stage, seedIdentity, stageIdentity, owner, StagePhase.RemovedSettings)
    await rm(path.join(seed, "config.json"), { force: true })
    await writeStageState(stage, seedIdentity, stageIdentity, owner, StagePhase.RemovedConfig)
  } finally {
...
const main = async (): Promise<void> => {
  const [seedArgument, marketplace, plugin, expectedVersion, ...extra] = process.argv.slice(2)
  if (
    ...
    fail("expected exactly 4 arguments: seed marketplace plugin expected-version")
  }
  if (!path.isAbsolute(seedArgument)) fail("seed path must be absolute")
  if (path.resolve(seedArgument) !== seedArgument || seedArgument === path.parse(seedArgument).root)
    fail("seed path is unsafe")
  ...
  if (!(await stat(expectedSourceArgument)).isDirectory()) fail("native marketplace path must be a directory")
  const expectedSource = await realpath(expectedSourceArgument)
  const forbiddenBuildPaths = [
    ...new Set([...knownBuildOnlyPaths, seedArgument, seed, expectedSourceArgument, expectedSource]),
  ...
  try {
    ...
    if (settingsFile === undefined) {
      assertExactJson(
        JSON.parse(await readFile(path.join(seed, "managed-lock.json"), "utf8")),
        marker,
        "managed-lock.json",
      )
    }
    ...
    const pluginFiles = managedEntries.filter((entry) => entry.kind === ManagedKind.File)
    const managedFiles = [...pluginFiles.map((entry) => entry.path), "managed-lock.json", "managed-settings.json"].sort(
      lexical,
    )
    ...
    await releaseOwnedLock(finalizationLock, LockKind.Main, seedIdentity, buildRoot)
  }
}
