...
import { syncSnapshot, verifyTarget } from "../../../scripts/floating-skills.ts"
import { manageManualSkill } from "../manual-skills.ts"
...
const seedSnapshot = async (cache: string, version = 1, includeManual = true) => {
...
test("manual library rejects discovery-visible paths and symlinked destinations", async (context) => {
  ...
  await assert.rejects(
    manageManualSkill({
      ...options,
      command: "ensure",
      library: path.join(root, "profile/home/library"),
    }),
...
test("fresh checks detect manual-only updates without changing cache or deployed copies", async (context) => {
  ...
  const fakeManager = path.join(root, "manager.ts")
  await write(
    fakeManager,
    `
...
`,
  ...
  const child = spawn(
    bunExecutable(),
    bunArguments(path.join(repository, "prototypes/trellage-claude-common/manual-skills.ts"), [
      fakeManager,
      "ensure",
      catalogPath,
      options.cache,
  ...
  try {
    await Promise.race([
      ...
      closed.then(() => {
        throw new Error("Manual helper exited before starting")
      }),
    ])
    child.kill("SIGTERM")
