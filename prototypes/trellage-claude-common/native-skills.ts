...
export type FloatingSkillsManager = typeof import("../../scripts/floating-skills.ts")
export type SkillPair = readonly [cache: string, target: string, excluded?: readonly string[]]

function fail(message: string): never {
  throw new Error(message)
}

export const statusIfPresent = async (candidate: string) => {
  try {
...
const requireOwnedEntry = (candidate: string, status: Stats) => {
  if (status.isSymbolicLink()) fail(`unsafe skills path (symlink): ${candidate}`)
  const uid = process.getuid?.()
  if (uid === undefined) fail("Native skills require operating-system user ownership checks")
  if (status.uid !== uid) fail(`skills path is not owned by the current user: ${candidate}`)
...
const expectedDirectoryPath = async (candidate: string) => {
...
export const requireDirectory = async (candidate: string, label: string) => {
...
export const requireFile = async (candidate: string) => {
...
const requireManagedTree = async (candidate: string): Promise<void> => {
...
const requireTarget = async (target: string, names: readonly string[]) => {
...
const readManagedManifest = async (candidate: string, legacy = false) => {
...
const legacyShowMe = async (target: string) => {
...
const targetManagedNames = async (target: string) => {
...
const requireTargetLock = async (target: string) => {
...
export const loadSkillsManager = async (managerPath: string): Promise<FloatingSkillsManager> => {
...
const bundlesForCache = (cache: string) => {
  switch (path.basename(cache)) {
    ...
    case "guide-prompt-master-skills":
      return ["guide-prompt-master"]
    default:
      fail(`unknown skill cache variant: ${cache}`)
  }
}
...
const installHelper = async (runtimeRoot: string, includeManual = false) => {
  ...
  const helpers = includeManual ? ["native-skills.ts", "manual-skills.ts"] : ["native-skills.ts"]
...
const main = async (args: readonly string[]) => {
  ...
  if (
    ...
    first !== undefined &&
    second !== undefined &&
    ["--install", "--install-manual"].includes(first)
  ) {
    return installHelper(second, first === "--install-manual")
  }
  ...
  if (
    manager === undefined ||
    command === undefined ||
    !["--sync", "--check", "--fresh"].includes(command) ||
    paths.length === 0 ||
    paths.length % 2 !== 0
  ) {
...
if (import.meta.main) {
  main(process.argv.slice(2)).catch((error) => {
    process.stderr.write(`native skills update: ${error instanceof Error ? error.message : String(error)}\n`)
    process.exitCode = 1
  })
}
